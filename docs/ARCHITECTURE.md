...
No backend, storage, analytics or auth is included. See `src/services/README.md`.
...
src/
  ...
  services/              data access — empty until a backend is chosen
...
accent or switching `primaryFollowsAccent` re-skins every component.
...
src/features/orders/
  ...
  hooks/useOrders.ts            calls src/services, returns AsyncState<Order[]>
...
- Feature hooks are the only place that knows about services.
...
1. Create the route file under `app/` (inside `(tabs)` or on the root stack).
2. Compose `<Screen header={<Header … />}>` with feature components.
3. Handle loading / empty / error / disabled states.
4. Check both color schemes; add new primitives to the Gallery.
